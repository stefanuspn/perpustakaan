...
function form_edit(id){
   $('#ModalEditAnggota form')[0].reset();
   $.ajax({
      url : "anggotaajax?action=form_data&uid="+id,
      type : "GET",
      dataType : "JSON",
      success : function(data){
         $('#ModalEditAnggota').modal('show');
         $('.modal-title').text('Edit Anggota');
         $('#uid').val(data.uid).attr('readonly',true);
         $('#id_anggota').val(data.id_anggota).attr('readonly',true);
         $('#nama').val(data.nama);
...
function delete_data(id){
   if(confirm("Apakah yakin data akan dihapus?")){
      $.ajax({
         url : "anggotaajax?action=delete&uid="+id,
         type : "GET",
         success : function(data){
...
function ExportExcel(){
  ...
  if (result.dismiss === 'timer') {
   window.location.href="laporananggotaexcel";
  }
})
  }
})
